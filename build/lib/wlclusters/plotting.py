...
#     I done after fitting, can optionally compute shear profiles in two scenarios :
...
#             mask = (rm >= min(wldata.rin_wl)) & (rm <= max(wldata.rout_wl)) # cutting the radial extrapolation
...
#             pmod = np.array((chains[0][1], chains[0][2])).T
#             gplus_results = []
#
#             for i in tqdm(range(len(chains[0][1]))):
...
def wldata_from_ID(lens_id,
                   ...
                   cosmo=None):
    """
    Function to retrieve weak lensing data and optionally compute shear profiles.
    ...
    # Extract relevant data based on lens_id
    results_id = results[np.isin(results['ID'], lens_id)]
    shear_id = shear_profiles[np.isin(shear_profiles['ID'], lens_id)]
    z_cl = cluster_cat[np.isin(cluster_cat['ID'], lens_id)]['z_p']
    rin = shear_id['rin']
    ...
    if return_shear:
        # Case for median parameters model
        if return_shear_model == 'median parameters':
            ...
            # Return data including masked shear profile
            return wldata, gplus_med[mask], rm[mask]

        # Case for enveloppe model
        elif return_shear_model == 'enveloppe':
            # Check if all_chains is provided
            if all_chains is None:
                print('Error: posterior chains are needed to compute the enveloppe.')
                return

            # Extract chains corresponding to the lens_id
            chains = all_chains[np.isin(all_chains['ID'], lens_id)]
            # Sample 500 rows randomly from the chains
            ...
            # Return data including all masked shear profiles
            return wldata, gplus_results, rm[mask]

    # Return wldata object if shear profiles are not requested
    return wldata